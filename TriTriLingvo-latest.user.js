...
(function() {
	...
	function Lingvo(_TriEsprimo){
		if (_TriEsprimo.indexOf("*") != -1){
			...
			if (v == _TriEsprimo.length){return true}else{return false}
		}else{
			let curl = window.location.href.replace(/https?:\/\//,"");
			if (curl[curl.length-1] == "/"){curl = curl.slice(0,curl.length-1)}
			if (_TriEsprimo == curl){return true}else{return false}
		}
	}
	//取得用户样式 取得索引 加载
	Trilato = GM_getValue("Trilato",Trilato);
	try{
		for (let tri = 0; tri < Trilato.length; tri++){
			if (Lingvo(Trilato[tri].TriEsprimo)){
				var Trindex = tri,_Trindex;
				var TriCss = Trilato[tri].TriLingvo;
				if (/^\d{1}$/.test(TriCss)){
					_Trindex = parseInt(TriCss);
					TriCss = Trilato[_Trindex].TriLingvo;
	...
	}catch(err){if(Trilato[0].TriLingvoEstasEll){console.log(err)}}
...
.Vzin_config{width: 300px; height: 650px !important; position: fixed; top: 5%; right: 8%; background-color: rgba(255, 255, 255, 0.8); font-size: 120%; color: #fa7298; resize: none; outline: none; border-radius: 16px; border-color: white; padding: 15px; z-index: 99999}
...
<textarea id="Vzin_config" class="Vzin_config" onfocus="document.querySelector('#Vzin-joinus').style.display= 'block'" onkeyup="document.querySelector('#Vzin_style').innerHTML = JSON.parse(this.value)[` + Mi + `].TriLingvo" type="input">` + JSON.stringify(Trilato).replace(/,/g,",\n").replace(/\},/g,"},\n").replace(/Ell":1,/g,'Ell":1,\n') + `</textarea>
</div><style id="Vzin_style" type="text/css">` + TriCss + `</style>`
		}});
